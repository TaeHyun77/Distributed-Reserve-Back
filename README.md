...
좌석 예약 시스템이 실제 티켓 오픈 상황에서 견딜 수 있는 참가열(동시 사용자) 크기를 확정하기 위해, 아래 4가지 테스트를 순차적으로 진행했습니다

[ 부하 테스트 구성 ]

| **테스트** | **목적** |
| --- | --- |
| Closed-loop N 스윕 | 처리 가능한 무릎점(용량) 탐색 |
| 매진 시나리오 | 좌석 경합 시 정합성(이중 예약 여부) 검증 |
| 스파이크 테스트 | 순간적인 트래픽 폭증 대응력 검증 |
| 소크 테스트 | 확정 N에서 장시간 지속 부하 안정성 검증 |
...
N 스윕에서 찾은 참가열 크기가 짧은 구간에서만 유효한 값은 아닌지 확인하기 위해, 확정 N을 기준으로 30분~1시간 동일 부하를 지속하며 시간에 따른 성능 저하나 누수가 없는지 검증했습니다
